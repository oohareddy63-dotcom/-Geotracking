...
const EmployeeDashboard = ({ user, onLogout }) => {
  ...
  const handleCreateTask = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/tasks`, {
        ...
        location: {
          latitude: parseFloat(newTask.location.latitude),
          longitude: parseFloat(newTask.location.longitude),
          address: newTask.address
        },
        geoFenceRadius: parseInt(newTask.geoFenceRadius),
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }} className="fade-in-up">
            ...
            >
              ...
              {tasks.length === 0 ? (
                <Alert severity="info" className="glass-card" sx={{ border: 'none', backdropFilter: 'none' }}>
                  You don't have any assigned tasks yet. Create your first task!
                </Alert>
              ) : (
      ...
      {/* Create Task Dialog */}
      <Dialog open={openCreateTaskDialog} onClose={() => setOpenCreateTaskDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle className="glass-card" sx={{ m: 0, p: 2, background: 'var(--bg-card)', backdropFilter: 'none', border: 'none', color: 'var(--text-primary)' }}>
          Create New Task
        </DialogTitle>
        <DialogContent dividers className="glass-card" sx={{ p: 3, background: 'var(--bg-card)', backdropFilter: 'none', border: 'none' }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                label="Latitude"
                type="number"
                value={newTask.location.latitude}
                onChange={(e) => setNewTask({ 
                  ...newTask, 
                  location: { ...newTask.location, latitude: e.target.value } 
                })}
                className="input-glass"
                sx={{ mb: 2 }}
              />
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                label="Longitude"
                type="number"
                value={newTask.location.longitude}
                onChange={(e) => setNewTask({ 
                  ...newTask, 
                  location: { ...newTask.location, longitude: e.target.value } 
                })}
                className="input-glass"
                sx={{ mb: 2 }}
              />
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => setNewTask({ ...newTask, geoFenceRadius: e.target.value })}
                className="input-glass"
                sx={{ mb: 2 }}
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <FormControl fullWidth className="input-glass" sx={{ mb: 2 }}>
        ...
        <DialogActions className="glass-card" sx={{ p: 2, background: 'var(--bg-card)', backdropFilter: 'none', border: 'none' }}>
          <Button onClick={() => setOpenCreateTaskDialog(false)} className="input-glass" sx={{ borderRadius: '8px', minWidth: '80px' }}>
            Cancel
          </Button>
          <Button onClick={handleCreateTask} className="btn-gradient" disabled={loading} sx={{ borderRadius: '8px', minWidth: '100px' }}>
